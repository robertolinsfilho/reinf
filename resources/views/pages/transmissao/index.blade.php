...
<div class="card mb-3">
    <div class="card-header d-flex justify-content-between align-items-center">
        <span><i class="bi bi-cloud-upload me-2"></i>Enviar XMLs — {{ $competencia['razao_social'] }} | {{ $competencia['periodo'] }}</span>
                ...
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="font-monospace small">{{ $a['nome_arquivo'] }}</td>
        ...
        <form method="POST" action="/transmissao/excluir-rfb">
            ...
            <div class="table-responsive mb-3">
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody>
                        @foreach($transmitidos as $a)
                        <tr>
                            <td><input type="checkbox" name="arquivos[]" value="{{ $a['id'] }}" class="chk-tx"></td>
                            <td><span class="badge bg-primary">{{ $a['evento'] ?? '—' }}</span></td>
                            <td class="font-monospace small">{{ $a['nome_arquivo'] }}</td>
                            <td class="font-monospace small text-success">{{ $a['nr_recibo_retornado'] }}</td>
                            <td class="small text-muted">{{ date('d/m/Y H:i', strtotime($a['created_at'])) }}</td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <table class="table table-sm table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-size:.82rem">{{ $h['razao_social'] ?? '' }}</td>
                        ...
                        <td class="font-monospace small">{{ $h['protocolo'] ?? '—' }}</td>
                        <td class="small" title="{{ $h['descricao_retorno'] ?? '' }}">
                            {{ mb_strimwidth($h['descricao_retorno'] ?? '—', 0, 40, '…') }}
                        </td>
                        <td>
                            @if($h['sucesso'])
