...
<div class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        <a href="/dashboard" class="nav-item {{ str_contains(request()->getRequestUri(), 'dashboard') ? 'active' : '' }}">
            <i class="bi bi-speedometer2"></i> Dashboard
        </a>
        <a href="/contribuintes" class="nav-item {{ str_contains(request()->getRequestUri(), 'contribuinte') ? 'active' : '' }}">
            <i class="bi bi-building"></i> Contribuintes
        </a>
        <a href="/competencias" class="nav-item {{ str_contains(request()->getRequestUri(), 'competencia') ? 'active' : '' }}">
            <i class="bi bi-calendar3"></i> Competências
        </a>
        <a href="/processos" class="nav-item {{ str_contains(request()->getRequestUri(), 'processo') ? 'active' : '' }}">
            <i class="bi bi-folder2-open"></i> R-1070 Processos
        </a>

        <div class="nav-label">PREVIDÊNCIA (INSS)</div>
        <a href="/eventos/r2010" class="nav-item sub {{ str_contains(request()->getRequestUri(), 'r2010') ? 'active' : '' }}">
            <i class="bi bi-arrow-right-short"></i> R-2010 Serv. Tomados
        </a>
        <a href="/eventos/r2020" class="nav-item sub {{ str_contains(request()->getRequestUri(), 'r2020') ? 'active' : '' }}">
            <i class="bi bi-arrow-right-short"></i> R-2020 Serv. Prestados
        </a>
        <a href="/eventos/r2055" class="nav-item sub {{ str_contains(request()->getRequestUri(), 'r2055') ? 'active' : '' }}">
            <i class="bi bi-arrow-right-short"></i> R-2055 Aq. Prod. Rural
        </a>
        <a href="/eventos/r2060" class="nav-item sub {{ str_contains(request()->getRequestUri(), 'r2060') ? 'active' : '' }}">
            <i class="bi bi-arrow-right-short"></i> R-2060 CPRB
        </a>

        <div class="nav-label">IRRF / CSRF (DIRF)</div>
        <a href="/eventos/r4010" class="nav-item sub {{ str_contains(request()->getRequestUri(), 'r4010') ? 'active' : '' }}">
            <i class="bi bi-arrow-right-short"></i> R-4010 Pagtos PF
        </a>
        <a href="/eventos/r4020" class="nav-item sub {{ str_contains(request()->getRequestUri(), 'r4020') ? 'active' : '' }}">
            <i class="bi bi-arrow-right-short"></i> R-4020 Pagtos PJ
        </a>

        <div class="nav-label">PROCESSAMENTO</div>
        <a href="/importar" class="nav-item {{ str_contains(request()->getRequestUri(), 'importar') ? 'active' : '' }}">
            <i class="bi bi-file-earmark-excel"></i> Importar Excel
        </a>
        <a href="/gerar" class="nav-item {{ str_contains(request()->getRequestUri(), 'gerar') ? 'active' : '' }}">
            <i class="bi bi-file-earmark-code"></i> Gerar XML
        </a>
        <a href="/gerar/xsd" class="nav-item sub {{ str_contains(request()->getRequestUri(), '/gerar/xsd') ? 'active' : '' }}">
            <i class="bi bi-arrow-right-short"></i> Status XSDs
        </a>
        <a href="/transmissao" class="nav-item {{ str_contains(request()->getRequestUri(), 'transmissao') ? 'active' : '' }}">
            <i class="bi bi-send"></i> Transmissão
        </a>
        <a href="/certificados" class="nav-item {{ str_contains(request()->getRequestUri(), 'certificado') ? 'active' : '' }}">
            <i class="bi bi-shield-lock"></i> Certificado A1
        </a>

        <div class="nav-label">CONSULTAS</div>
        <a href="/naturezas" class="nav-item {{ str_contains(request()->getRequestUri(), 'naturezas') ? 'active' : '' }}">
            <i class="bi bi-tags"></i> Naturezas de Rendimento
        </a>

        @if(($usuario['perfil'] ?? '') === 'admin')
        <div class="nav-label">ADMIN</div>
        <a href="/usuarios" class="nav-item {{ str_contains(request()->getRequestUri(), 'usuario') ? 'active' : '' }}">
            <i class="bi bi-people"></i> Usuários
        </a>
        @endif
    </nav>
    <div class="sidebar-footer">
        <a href="/perfil"><i class="bi bi-person-circle"></i> {{ $usuario['nome'] }}</a>
